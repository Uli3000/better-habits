...
interface AddGoalDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (title: string, emoji: string, weeklyTarget: number) => void;
}

const EMOJI_OPTIONS = ['🏃','💪','🧘','📚','✍️','🧠','🎯','⏳','🔥','⚡','🌱','🌿','💧','🍎','😴','📈','🗓️','🧹','💬','✨','💻','🖥️','⌨️','📝','📖','🎓','🌍','🗣️','🔤','📊'];

export function AddGoalDialog({ isOpen, onClose, onAdd }: AddGoalDialogProps) {
  ...
  const handleSubmit = () => {
    if (title.trim()) {
      onAdd(title.trim(), emoji, weeklyTarget);
      setTitle('');
      setEmoji('🎯');
      setWeeklyTarget(5);
      onClose();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 animate-fade-in">
      ...
      <div className="glass relative w-full max-w-md rounded-2xl p-6 animate-slide-up">
        ...
        <h3 className="text-xl font-bold text-foreground mb-6">Nueva Meta</h3>
